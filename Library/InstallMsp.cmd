...
:: Name		: InstallMsp.cmd
::
:: Funktion	: Installation einer MSP-Datei
::
...
FOR %%i IN (*.msp) DO (
	SET MSPSOURCE_FullName=%%~fi
	SET MSPSOURCE_Name=%%~ni
)

IF NOT DEFINED MSPSOURCE_FullName (
	CALL "%LIB%\Tee.cmd" Keine MSP-Datei gefunden in %CD%
	GOTO :END
)

SET MSPLOG=%LOG%\%MSPSOURCE_Name%.log

::IF EXIST "%MSPLOG%" (
CALL "%LIB%\CheckMsp.cmd" %MSPSOURCE_Name%
IF NOT ERRORLEVEL 1 (
	CALL "%LIB%\Tee.cmd" Schon installiert - %MSPSOURCE_Name%
	GOTO :END
)

:: Optionale Settings aus InstallMsp.ini lesen (setzt TRANSFORMS und PARAMETERS)
FOR %%i IN (%~n0.ini) DO CALL "%LIB%\ReadSettings.cmd" %%i

FOR %%i IN (%TRANSFORMS%) DO SET TRANSFORMS=TRANSFORMS="%%~fi"

SET ARGUMENTLIST=/update "%MSPSOURCE_FullName%" %TRANSFORMS% %PARAMETERS% /QN REBOOT=ReallySuppress /liewa "%MSPLOG%"

CALL "%LIB%\Tee.cmd" msiexec wird gestartet mit %ARGUMENTLIST%
START /WAIT msiexec.exe %ARGUMENTLIST%
SET MSIEXIT=%ERRORLEVEL%

CALL "%LIB%\Tee.cmd" %MSPSOURCE_Name%: Exitcode %MSIEXIT%
IF %MSIEXIT%==0 (
